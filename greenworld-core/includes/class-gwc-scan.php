...
final class GWC_Scan {
	...
	public function form( $atts = array() ): string {
		...
		if ( ! $printed_css ) {
			$printed_css = true;
			echo '<style>.gw-scan-form{display:grid;gap:.6rem;max-width:520px}.gw-scan-form label{display:block;font-size:.9rem;font-weight:600;color:#14421f}.gw-scan-form input,.gw-scan-form textarea{width:100%;padding:.6rem .7rem;border:1px solid rgba(0,0,0,.15);border-radius:8px;font:inherit;margin-top:.2rem;box-sizing:border-box}.gw-scan-hp{position:absolute;left:-9999px}.gw-scan-notice{padding:.7rem .9rem;border-radius:8px;margin:0 0 .8rem}.gw-scan-notice--ok{background:#e7f6ec;color:#14612b}.gw-scan-notice--err{background:#fdecec;color:#8a1c1c}</style>';
		}
		if ( 'ok' === $status ) {
